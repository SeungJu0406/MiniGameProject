...
public class Test : MonoBehaviour
{
    ...
    private void Start()
    {
        ItemDic.Instance.Init();
    }
    private void Update()
    {
        ingredients.Clear();
        ItemData rock = ItemDic.Instance.GetInfo(ItemType.Rock);
        AddIngredint(rock);

        ItemData water = ItemDic.Instance.GetInfo(ItemType.Water);
        AddIngredint(water);
        //AddIngredint(water);

        ItemData wood = ItemDic.Instance.GetInfo(ItemType.Wood);
        AddIngredint(wood);

        ingredients.Sort((s1, s2) => s1.item.type.CompareTo(s2.item.type));
        string key = RecipeDic.Instance.GetKey(ingredients.ToArray());

        if (RecipeDic.Instance.recipeDic.ContainsKey(key))
        {
            CraftingItemInfo result = RecipeDic.Instance.GetInfo(key);
